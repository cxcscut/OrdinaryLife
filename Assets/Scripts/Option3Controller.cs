...
public class Option3Controller : MonoBehaviour {

	private Rect frame;

	public new Camera camera;

	public GameObject option3_clicked;
		...
		if (!GlobalVariables.Shot5Finished && GlobalVariables.Shot9Active) {
			...
			if (frame.Contains (new Vector2 (mouse_pos.x, mouse_pos.y), true) || frame.Contains (new Vector2 (touch_pos.x, touch_pos.y), true)) {
				...
				if (Input.GetMouseButtonDown (0) || (Input.touchCount > 0 && Input.GetTouch (0).phase == TouchPhase.Began)) {
					...
					option3_clicked.GetComponent<SpriteRenderer> ().enabled = true;
				}

				if (Input.GetMouseButtonUp (0) || (Input.touchCount > 0 && Input.GetTouch (0).phase == TouchPhase.Ended)) {
					GetComponent<SpriteRenderer> ().enabled = true;
					option3_clicked.GetComponent<SpriteRenderer> ().enabled = false;
